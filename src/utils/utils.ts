...
import {IShipPosition, SellCoordinate} from "../types/entities/sellCoordinate";
import {IShip, Ship} from "../types/entities/ship";
...
const canShipBePlaced = (ship: IShip, shipsMatrix: BattlefieldMatrixType): boolean => {
    if (ship.direction) {
...
const randomFreeSellGenerator = (shipsMatrix: BattlefieldMatrixType): IShipPosition => {
...
export const battlefieldMatrixGenerator = (ships: Array<IShip>): BattlefieldMatrixType => {
...
export const lastShotHandler = (
    shipsMatrix: BattlefieldMatrixType,
    playerKilledShips: Array<IShipPosition>,
    lastShotX: number,
    lastShotY: number
): Nullable<IShipKillAttackResult> => {
    try {
        const lastAttackedSell: SellType = shipsMatrix[lastShotY][lastShotX];
        const killedShipSells: Array<IShipPosition> = [];
        let aroundShotsCoords: Array<IShipPosition> = [];
...
export const randomAttackGenerator = (shipsMatrix: BattlefieldMatrixType): IShipPosition => {
    let randomX = Math.round(Math.random() * (BATTLEFIELD_MATRIX_SIZE - 1));
    let randomY = Math.round(Math.random() * (BATTLEFIELD_MATRIX_SIZE - 1));
...
export const botShipsGenerator = (): Array<IShip> => {
    const botShips: Array<IShip> = [];
